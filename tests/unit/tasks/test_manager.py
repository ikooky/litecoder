...
async def completed_task(
    ...
    await manager.assign_and_start(task_id, agent_id)
...
@pytest.mark.asyncio
async def test_assignment_rejects_task_until_dependencies_complete(
    manager: TaskManager,
) -> None:
    ...
    with pytest.raises(TaskBlocked) as error:
        await manager.assign_and_start("work", "agent-a")

    assert error.value.blocking_ids == ["dep"]
    await manager.assign_and_start("dep", "agent-b")
    await manager.complete("dep", "agent-b")
    assigned = await manager.assign_and_start("work", "agent-a")
...
async def test_wrong_owner_cannot_complete_task(manager: TaskManager) -> None:
    ...
    await manager.assign_and_start("t1", "agent-a")
...
@pytest.mark.asyncio
async def test_separate_task_managers_serialize_assignments_with_file_lock(
    tmp_path: Path,
) -> None:
    ...
    results = await asyncio.gather(
        _assignment_status(first, "t1", "agent-a"),
        _assignment_status(second, "t1", "agent-b"),
    )

    assert sorted(results) == ["assigned", "not-assignable"]
    final = store.read("t1")
